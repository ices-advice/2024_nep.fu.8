...
stock.object="nephup.ff.2022.rdata"

#Previous year international landings
old.international=paste0(datadir,"international.landings_2022.csv")
...
# Create stock object
nephup.ff<- nephup(wdir=datadir, stock.object=stock.object, lfile="Frsff.txt", filenames="File_listFF.txt", bmsfile="BMSff.txt")
#round BMS to nearest tonne
attr(nephup.ff,"bms")<- round(attr(nephup.ff,"bms"))
#  Effort - new Katy Barratt effort data 
nephort.ff <- list(days=nephort(wdir = datadir, eff.file = "eff_FF_days.txt"), kwdays=nephort(wdir = datadir, eff.file = "eff_FF_kwdays.txt"))
save(nephort.ff,nephup.ff,file=paste0(datadir,"nephup.ff.", catch.year, ".rdata"))
...
tables(datadir,nephup.ff, nephort.ff$days, f.u="forth")
...
create.international.landings(wdir=datadir, fu="nep.fu.8", new.data.year=catch.year, 
...
invisible(mean.weight(datadir,stock.list=list(nephup.ff)))
invisible(mean.wt.disc(datadir, stock.list=list(nephup.ff)))
mean.weight.catches(wk.dir=datadir, stock=nephup.ff, MCS=25)
